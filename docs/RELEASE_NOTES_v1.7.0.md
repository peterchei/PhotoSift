# PhotoSift v1.7.0 Release Notes
...
### New test file: `tests/test_low_resolution.py`

27 tests (17 unit + 10 integration) for the new Low Resolution Detector.
...
| v1.7.0  | 89    | 2 (same pre-existing import errors, unchanged) |
...
`LowResolutionDetection.py` and `LowResolutionGUI.py` require no new data files or model bundles — no `PhotoSift.spec` changes needed for the new feature.
...
- Attach to GitHub release: `PhotoSift.exe`, `PhotoSift_Setup.exe`, `PhotoSift.msix`

---

*PhotoSift v1.7.0 — 2026-02-28*
